...
public class rounda_lane_4_add_thread extends Thread {
    public Handler handler;
    public int[] result;
    private String densityType, seconds;
    private String TAG = "rounda_lane_4_add_thread";
    private TextView con_lane_4, ai_lane_4;

    rounda_lane_4_add_thread(String seconds, String densityType, int[] result, TextView con_lane_4, TextView ai_lane_4) {
        this.seconds = seconds;
        this.densityType = densityType;
        this.result = result;
        this.con_lane_4 = con_lane_4;
        ...
        if (SimulationActivity.running) {
            ...
            if (densityType.equals("Heavy Traffic")) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        if (SimulationActivity.running) {
                            handler.post(new Runnable() {
                                @Override
                                public void run() {
                                    int con_lane_val = Integer.parseInt(con_lane_4.getText().toString());
                                    int ai_lane_val = Integer.parseInt(ai_lane_4.getText().toString());
                                    if (con_lane_val < 10) {
                                        con_lane_val = con_lane_val + 1;
                                        con_lane_4.setText(String.valueOf(con_lane_val));
                                    }
                                    if (ai_lane_val < 10) {
                                        ai_lane_val = ai_lane_val + 1;
                                        ai_lane_4.setText(String.valueOf(ai_lane_val));
                                    }
                                }
                            });
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        con_lane_4.setText(String.valueOf(result[0]));
                        ai_lane_4.setText(String.valueOf(result[0]));
                    }
                });
                SystemClock.sleep(add_time);
                while (SimulationActivity.running) {
