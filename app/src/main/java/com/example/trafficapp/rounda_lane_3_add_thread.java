...
public class rounda_lane_3_add_thread extends Thread {
    public Handler handler;
    public int[] result;
    private String densityType, seconds;
    private String TAG = "rounda_lane_3_add_thread";
    private TextView con_lane_3, ai_lane_3;

    rounda_lane_3_add_thread(String seconds, String densityType, int[] result, TextView con_lane_3, TextView ai_lane_3) {
        this.seconds = seconds;
        this.densityType = densityType;
        this.result = result;
        this.con_lane_3 = con_lane_3;
        ...
        if (SimulationActivity.running) {
            ...
            if (densityType.equals("Heavy Traffic")) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        if (SimulationActivity.running) {
                            handler.post(new Runnable() {
                                @Override
                                public void run() {
                                    int con_lane_val = Integer.parseInt(con_lane_3.getText().toString());
                                    int ai_lane_val = Integer.parseInt(ai_lane_3.getText().toString());
                                    if (con_lane_val < 10) {
                                        con_lane_val = con_lane_val + 1;
                                        con_lane_3.setText(String.valueOf(con_lane_val));
                                    }
                                    if (ai_lane_val < 10) {
                                        ai_lane_val = ai_lane_val + 1;
                                        ai_lane_3.setText(String.valueOf(ai_lane_val));
                                    }
                                }
                            });
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        con_lane_3.setText(String.valueOf(result[0]));
                        ai_lane_3.setText(String.valueOf(result[0]));
                    }
                });
                SystemClock.sleep(add_time);
                while (SimulationActivity.running) {
