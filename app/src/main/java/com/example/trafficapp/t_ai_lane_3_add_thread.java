...
public class t_ai_lane_3_add_thread extends Thread {
    int seconds;
    public Handler handler;
    private String densityType;
    private String TAG = "t_con_lane_3_add_thread";

    t_ai_lane_3_add_thread(int seconds, String densityType) {
        this.seconds = seconds;
        this.densityType = densityType;
    }
            ...
            if (densityType.equals("Heavy Traffic")) {
                for (int i = 0; i < 10; i++) {
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            int lane_val = Integer.parseInt(SimulationActivity.tjunction_ai_lane_3_in.getText().toString());
                            if (lane_val < 10) {
                                lane_val = lane_val + 1;
                                SimulationActivity.tjunction_ai_lane_3_in.setText(String.valueOf(lane_val));
